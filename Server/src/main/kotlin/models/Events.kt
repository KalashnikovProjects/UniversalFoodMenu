...
//    data class ChangeImageEvent(val id: Int, val element: ImageItem) : Events
    ...
    @Serializable
    data class AddDesignEvent(val element: DesignItemWithScreenId) : Events
    @SerialName("change_design_event")
    @Serializable
    data class ChangeDesignEvent(val id: Int, val element: DesignItemWithScreenId) : Events
    ...
    @Serializable
    data class ReloadCategorizedFoodItems(val c: FoodItemsCategorized) : Events

    @SerialName("reload_screens")
    @Serializable
    data class ReloadScreens(val items: List<TVScreen>) : Events

    @SerialName("data_loaded")
    @Serializable
    object DataLoaded : Events
}